...
MyPolygon* calcAnalyticalFF()
{
  ...
  for(unsigned int i = 0; i < polygons.size(); i++)
  {
	  ...
	  polygons[i]->formF =
		  ...
		  / (M_PI * dot(dist, dist));
	  polygons[i]->formF *= polygons[i]->area;
  }
...
bool distributeEnergy(MyPolygon* maxP)
{
  ...
  for(unsigned int i = 0; i < polygons.size(); i++)
  {
	  ...
	  Vec3f value = polygons[i]->formF * maxP->unshot_rad * polygons[i]->diffuse* maxP->area / polygons[i]->area;
...
void colorReconstruction()
{
  ...
  for(unsigned int i = 0; i < polygons.size(); i++)
  {
		polygons[i]->color = polygons[i]->rad*M_1_PI;
...
MyPolygon* calcFF(Hemicube* hemicube)
{
  ...
  Vec3f normUp = normalize(p->center-vertices[p->vertex[0]]->position);
	...
	float c =p->area/(hemicube->rendersize*hemicube->rendersize);
